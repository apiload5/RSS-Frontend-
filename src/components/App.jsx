...
const AdUnit = ({ type = "google", size = "responsive" }) => {
  ...
  return (
    <div className={`bg-${config.color}-50 dark:bg-${config.color}-900/20 border-2 border-dashed border-${config.color}-300 dark:border-${config.color}-700 rounded-lg p-4 text-center my-4`}>
      <div className={`text-${config.color}-600 dark:text-${config.color}-400 text-sm font-medium mb-1`}>
        {config.text} - Advertisement
      </div>
      <div className={`text-${config.color}-700 dark:text-${config.color}-300 text-xs`}>
        {size === "responsive" ? "Responsive Ad Unit" : `${size} Banner`}
      </div>
      <div className={`text-${config.color}-600 dark:text-${config.color}-500 text-xs mt-1`}>
        {config.sizes.join(" | ")}
      </div>
    </div>
  );
...
// Main App Component (Updated with landing page)
const App = () => {
  ...
  const handleViewFeed = (feedId, feedName) => {
    setSelectedFeed({ id: feedId, name: feedName });
    setCurrentView('feed-items');
  };
  ...
  if (currentView === 'feed-items' && selectedFeed) {
    return (
      <FeedItemsView
